...
mpc            = 2;
threshold = -1/eps;
loc_start = find(Z1 > threshold, 1)+lag;
...
X = [X, DATA(loc_start+1:loc_end,:)];

for i = 1:2*lag_tfp+1
      X(:,6+i) = DTFP_UTIL(loc_start-lag_tfp+i:loc_end-lag_tfp+i-1);
end
for l = 1:lag
...
% STLP
varlist = {'TFP','Real GDP', 'Real Consumption', 'Unemployment Rate','Real Wage','Hours','CPI Inflation','GDP Deflator'};
lags =4;
H = 20; %irfs horizon
mpc = 2;
...
IR_E = {IR_E_T, IR_E_G,IR_E_C, IR_E_U, IR_E_W, IR_E_H, IR_E_CPI, IR_E_DEF};
IR_R = {IR_R_T, IR_R_G,IR_R_C, IR_R_U, IR_R_W, IR_R_H, IR_R_CPI, IR_R_DEF};
IR_L = {IR_L_T, IR_L_G,IR_L_C, IR_L_U, IR_L_W, IR_L_H, IR_L_CPI, IR_L_DEF};
...
for j = 1: length(varlist)
    ...
    hold on
    if j == 4 || j == 7 || j == 8 %do not take cumsum
