...
for iii = 1:3
      ...
      if iii == 2
            ...
            lead_tfp       = 8; %number of leads of TFP
            lag            = 3;  %number of lags of control variables (other structural shocks)
            ...
            for i = 1:lag_tfp %When i = 1 TFP is contemporaneous
                  X(:,SX+i) = DTFP_UTIL(loc_start+2-i:loc_end-i+1);
            end
            SX = size(X,2);
            for i = 1:lead_tfp
                  X(:,SX+i) = DTFP_UTIL(loc_start+1+i:loc_end+i);
            end
            for l = 1:lag
            ...
            % STLP
            varlist = {'TFP','Real GDP', 'Real Consumption', 'Unemployment Rate','Real Wage','Hours','CPI Inflation','Real Investment'};
            lags =4;
            H = 20; %irfs horizon
            mpc = 2;
