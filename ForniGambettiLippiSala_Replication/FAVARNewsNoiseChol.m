function [irfs,irfsb,irf,irfb,sigma_a,sigma_aboot,bidielle,sh,ssh,wr] = FAVARNewsNoiseChol(X,Z,codeData,k,h,nrepli,variables,ind1,ind,H)

%disp('FAVARNewsNoiseChol');

% ind1: position of the variable hit by "news shock" (potential, G,...)
% ind : position of the "signal" variable (G exp, or confidence....)

%codeData2 = zeros(size(codeData));
codeData2 = codeData;
if nargin == 9
H = h;
end
...
%rr = [1./wr; r(abs(r)>1)];
%ccstar = [flipud(poly(rr)'); 0];
%ccstar = sum(cc)*ccstar/sum(ccstar);

%s = fminsearch('irfit',1,[],ccstar,squeeze(irf(variables(ind1),ind1,:)));
...
irfs = irfstruc;%CumImp(irfstruc, codeData2);
irf = irf2;
...
irfb = FAVARCholBoot(X,Z,variables,k,h,nrepli);
