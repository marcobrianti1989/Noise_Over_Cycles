...
for t=[ind1 indControl ind]
    for j=1:2;
        temp = squeeze(irfb_tfp_bench(t,j,1:ll,:));
        if t==2
            TEMP = [squeeze(irf_tfp_bench(t,j,1:ll)) 0*squeeze(irfsR4(t,j,1:ll)) squeeze(irfsR3(t,j,1:ll))];
       
        else
        TEMP = [squeeze(irf_tfp_bench(t,j,1:ll)) squeeze(irfsR4(t,j,1:ll)) squeeze(irfsR3(t,j,1:ll))];
        end
        lo=lo+1;
        ...
        set(a(1),'Color','k','LineWidth',2);%if lo==2;legend(a(1:3),'Benchmark specification II','Stock prices with spread','Alternative identification');end
        if t==2
            set(a(2),'Color','k');
        else
        ...
        axis([0 ll-1 min(min(prctile(irfb_tfp_bench(t,1:2,:,:),5,4)))-.1 max(max(prctile(irfb_tfp_bench(t,1:2,:,:),95,4)))+.1]);
...
if length(varseries)>2
    ...
    for t = [indGDP indC indI]
        for j = 1:2,
            ...
            b=fill([0:ll-1 ll-1:-1:0],[te4' flipud(te5)'],col2);set(b,'EdgeAlpha',0);
            a = plot(0:ll-1,[squeeze(TEMP) 0*te_med ]);
            set(a(1),'Color','k','LineWidth',2);%if lo==2;
               % legend(a(1:3),'Benchmark specification II','Stock prices with spread','Alternative identification');end
            set(a(2),'LineWidth',2,'LineStyle','--');
            set(a(3),'LineWidth',1,'LineStyle','-','Marker','x');
            set(a(end),'Color','k');
            TE = [te2 te3 te4 te5];
