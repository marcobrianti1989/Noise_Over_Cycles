function [IRF_E, IRF_R, IRF_L ] = stlp(y,x,u,fz,lags,H)
...
for h = 1:H
    ...
    if lags > 0 %which allows for controls
        for jj = 1:lags;
            if x(1)^2 > 0; 
            X  = [X, u(lags-jj+1:end-jj-h+1,:).*fz(lags+1:end-h+1,1), y(lags-jj+1:end-jj-h+1,:).*fz(lags+1:end-h+1,1),...
               x(lags-jj+1:end-jj-h+1,:).*fz(lags+1:end-h+1,1)];
